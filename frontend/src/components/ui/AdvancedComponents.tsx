    ...
    <nav className={`flex items-center gap-1.5 text-sm ${className}`} aria-label="Breadcrumb">
      {displayed.map((item, i) => (
        <React.Fragment key={i}>
          {i > 0 && <span className="text-gray-400 dark:text-gray-500 text-xs">{separator}</span>}
          {item.href ? (
            <a href={item.href}
              className="flex items-center gap-1 text-gray-500 hover:text-blue-500 dark:text-gray-400 transition-colors">
              {item.icon} {item.label}
            </a>
          ) : (
            <span className={`flex items-center gap-1 ${i === displayed.length - 1 ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-500 dark:text-gray-400'}`}>
              {item.icon} {item.label}
            </span>
          )}
        </React.Fragment>
        ...
        return (
          <div key={i}
            ...
            <div className={`flex ${isHorizontal ? 'items-center w-full' : 'flex-col items-center'}`}>
              {i > 0 && (
                <div className={`${isHorizontal ? 'flex-1 h-0.5' : 'w-0.5 h-6'} ${isCompleted ? 'bg-blue-500' : 'bg-gray-200 dark:bg-gray-700'}`} />
              )}
              ...
              {i < steps.length - 1 && (
                <div className={`${isHorizontal ? 'flex-1 h-0.5' : 'w-0.5 h-6'} ${i < currentStep ? 'bg-blue-500' : 'bg-gray-200 dark:bg-gray-700'}`} />
              )}
            </div>

            <div className={`${isHorizontal ? 'text-center mt-2' : 'flex-1'}`}>
              <div className={`text-sm font-medium ${isActive ? 'text-blue-500' : isCompleted ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>
                {step.label}
              </div>
              {step.description && (
            ...
            >
              <div className="flex flex-col items-center">
                <div className={`w-6 h-6 rounded-full ${item.color || 'bg-blue-500'} text-white flex items-center justify-center flex-shrink-0 ${variant === 'compact' ? 'w-4 h-4' : ''}`}>
                  {variant !== 'compact' && (item.icon || <div className="w-1.5 h-1.5 bg-white rounded-full" />)}
                </div>
                {i < items.length - 1 && (
              ...
              <div className={`pb-6 ${variant === 'compact' ? 'pb-3' : ''}`}>
                <div className="flex items-center gap-2">
                  <h4 className={`font-medium text-gray-900 dark:text-white ${variant === 'compact' ? 'text-xs' : 'text-sm'}`}>{item.title}</h4>
                  {item.time && <span className="text-[10px] text-gray-400">{item.time}</span>}
                </div>
                {item.description && <p className={`text-gray-500 mt-0.5 ${variant === 'compact' ? 'text-[10px]' : 'text-xs'}`}>{item.description}</p>}
    ...
    <div>
      <motion.div
        ...
        className={`flex items-center gap-1.5 py-1 px-2 rounded-lg cursor-pointer select-none transition-colors ${
          isSelected ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'
        }`}
  ...
  return (
    <div className={`relative ${className}`}>
      <button onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-300 dark:hover:border-blue-600 transition-colors">
        <div className="w-6 h-6 rounded-lg border border-gray-300 dark:border-gray-600 shadow-inner" style={{ backgroundColor: value }} />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-50 mt-2 p-3 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700"
          >
            ...
            {showInput && (
              <input type="text" value={value} onChange={e => onChange(e.target.value)}
                className="w-full px-2.5 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-sm font-mono text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500" />
            )}
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors hover:border-blue-300 dark:hover:border-blue-600 dark:border-gray-700"
      >
        {children || (
          <div>
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
        ...
        {onClearAll && (
          <button onClick={onClearAll} className="text-xs text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
            Clear all
          </button>
        )}
      </div>
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map(n => (
            <motion.div key={n.id} whileHover={{ backgroundColor: 'rgba(59,130,246,0.03)' }}
              className={`px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 cursor-pointer ${!n.read ? 'bg-blue-50/30 dark:bg-blue-500/5' : ''}`}
              onClick={() => onMarkRead?.(n.id)}>
              <div className="flex items-start gap-2">
                <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${typeDots[n.type]}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h4 className={`text-sm ${!n.read ? 'font-semibold text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{n.title}</h4>
                    <span className="text-[10px] text-gray-400 ml-2 flex-shrink-0">{n.time}</span>
                  </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            ...
            <kbd className="hidden md:inline-flex px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px] text-gray-500">ESC</kbd>
          </div>
          <div className="max-h-72 overflow-y-auto py-2">
            {Object.entries(grouped).map(([category, catItems]) => (
              <div key={category}>
                <div className="px-4 py-1 text-[10px] font-medium text-gray-500 uppercase tracking-wider">{category}</div>
                {catItems.map(item => (
                  <motion.button key={item.id}
                    ...
                    {item.shortcut && (
                      <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px] text-gray-500 font-mono">{item.shortcut}</kbd>
                    )}
                  </motion.button>
                ))}
              </div>
