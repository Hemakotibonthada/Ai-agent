...
export default function CodeEditor() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      {/* Top Bar */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between px-4 py-2 bg-nexus-bg border-b border-gray-800">
          ...
          <div className="flex items-center gap-1 ml-4">
            {files.map(f => (
              <motion.button key={f.id} whileHover={{ scale: 1.02 }} onClick={() => setActiveFile(f.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-lg text-xs transition-all ${activeFile === f.id ? 'bg-nexus-card text-white border-t-2 border-green-400' : 'text-nexus-muted hover:text-gray-200 hover:bg-gray-800/50'}`}>
                <FileCode className={`w-3 h-3 ${languageColors[f.language] || 'text-nexus-muted'}`} />
                <span>{f.name}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.button whileHover={{ scale: 1.05 }} className="p-1.5 hover:bg-nexus-card rounded-lg text-nexus-muted"><Save className="w-4 h-4" /></motion.button>
          <motion.button whileHover={{ scale: 1.05 }} className="p-1.5 hover:bg-nexus-card rounded-lg text-nexus-muted"><Settings className="w-4 h-4" /></motion.button>
        </div>
      </motion.div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showSidebar && (
          <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 200, opacity: 1 }} className="w-52 bg-gray-900/50 border-r border-gray-800 overflow-y-auto">
            <div className="p-3">
              <p className="text-xs text-nexus-muted uppercase tracking-wider mb-2">Explorer</p>
              <div className="space-y-0.5">
                <div className="flex items-center gap-1.5 px-2 py-1 text-xs text-nexus-muted">
                ...
                {files.map(f => (
                  <motion.button key={f.id} whileHover={{ x: 2 }} onClick={() => setActiveFile(f.id)}
                    className={`w-full flex items-center gap-1.5 px-4 py-1 text-xs rounded transition-all ${activeFile === f.id ? 'bg-nexus-card text-white' : 'text-nexus-muted hover:text-gray-200 hover:bg-gray-800/50'}`}>
                    <FileCode className={`w-3 h-3 ${languageColors[f.language] || 'text-nexus-muted'}`} />
                    <span>{f.name}</span>
            ...
            {lineNumbers && (
              <div className="w-12 bg-gray-900/30 border-r border-gray-800/50 overflow-hidden pt-2 select-none">
                {Array.from({ length: lineCount }, (_, i) => (
                  <div key={i} className="px-2 text-right text-xs text-nexus-muted leading-6">{i + 1}</div>
                ))}
              </div>
            )}
            {/* Code Area */}
            ...
            <div className="h-48 bg-gray-900/50 border-t border-gray-800 flex flex-col">
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-gray-800/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-nexus-muted font-medium">Console</span>
                  <span className="px-1.5 py-0.5 bg-nexus-card rounded text-[10px] text-nexus-muted">{consoleOutput.length}</span>
                </div>
                <div className="flex items-center gap-1">
                  <motion.button whileHover={{ scale: 1.1 }} onClick={() => setConsoleOutput([])} className="p-1 hover:bg-nexus-card rounded"><Trash2 className="w-3 h-3 text-nexus-muted" /></motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} onClick={() => setShowConsole(false)} className="p-1 hover:bg-nexus-card rounded"><Minimize2 className="w-3 h-3 text-nexus-muted" /></motion.button>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-0.5">
                {consoleOutput.map(entry => (
                  <div key={entry.id} className={`px-2 py-0.5 rounded ${entry.type === 'error' ? 'text-red-400 bg-red-500/5' : entry.type === 'warn' ? 'text-yellow-400 bg-yellow-500/5' : entry.type === 'info' ? 'text-blue-400' : 'text-nexus-muted'}`}>
                    <span className="text-nexus-muted">[{new Date(entry.timestamp).toLocaleTimeString()}]</span> {entry.message}
                  </div>
                ))}
              </div>
            </div>
