        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">NexusAI REST API v1.0 · {totalEndpoints} endpoints</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-500">Base URL: https://api.nexus.local/v1</span>
          </div>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <div className="w-52 shrink-0 space-y-4">
            ...
            <div className="space-y-1">
              <button onClick={() => setSelectedTag(null)} className={`w-full text-left px-3 py-2 text-xs rounded-lg ${!selectedTag ? 'bg-indigo-500/20 text-indigo-300' : 'text-gray-500 hover:text-gray-300'}`}>
                All Endpoints ({totalEndpoints})
              </button>
              {SAMPLE_GROUPS.map(g => (
                <button key={g.tag} onClick={() => setSelectedTag(g.tag === selectedTag ? null : g.tag)}
                  className={`w-full text-left px-3 py-2 text-xs rounded-lg flex items-center justify-between ${selectedTag === g.tag ? 'bg-indigo-500/20 text-indigo-300' : 'text-gray-500 hover:text-gray-300'}`}>
                  <span>{g.tag}</span>
              ...
              <motion.div key={group.tag} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gray-800/20 border border-gray-700/50 rounded-xl overflow-hidden">
                <button onClick={() => toggleGroup(group.tag)} className="w-full px-5 py-3 flex items-center justify-between hover:bg-gray-800/40 transition-colors">
                  <div className="flex items-center gap-2">
                    {expandedGroups.has(group.tag) ? <ChevronDown className="w-4 h-4 text-gray-500" /> : <ChevronRight className="w-4 h-4 text-gray-500" />}
                          ...
                          return (
                            <div key={key} className="border-b border-gray-700/20 last:border-b-0">
                              <button onClick={() => toggleEndpoint(key)} className="w-full px-5 py-3 flex items-center gap-3 hover:bg-gray-800/30 transition-colors">
                                ...
                                <span className="text-xs text-gray-500 ml-auto">{ep.summary}</span>
                                  ...
                                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                                    ...
                                    <div className="px-5 pb-4 space-y-4">
                                      ...
                                      {ep.params.length > 0 && (
                                        <div>
                                          ...
                                          <table className="w-full text-xs">
                                            <thead><tr className="text-gray-600 border-b border-gray-700/30">
                                            ...
                                            <tbody>
                                              {ep.params.map(p => (
                                                <tr key={p.name} className="border-b border-gray-700/15">
                                                  ...
                                                  <td className="py-1.5 pr-3 text-center">{p.required ? <Check className="w-3 h-3 text-green-400 mx-auto" /> : <span className="text-gray-600">—</span>}</td>
                                      ...
                                      <div className="flex gap-4">
                                        <div className="flex-1">
                                          <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-xs text-gray-500 uppercase tracking-wider">Response ({ep.responseStatus})</h4>
                                            <button onClick={() => copyToClipboard(ep.responseExample)} className="text-gray-600 hover:text-gray-300"><Copy className="w-3.5 h-3.5" /></button>
                                          </div>
                                      ...
                                      <div className="flex gap-2 pt-2 border-t border-gray-700/30">
                                        ...
                                        <button onClick={() => copyToClipboard(`curl -X ${ep.method} https://api.nexus.local${ep.path}`)} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-300 text-xs rounded-lg transition-colors">
        ...
        <AnimatePresence>
          {tryItResponse && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
              className="fixed bottom-6 right-6 w-96 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-50 overflow-hidden">
              <div className="px-4 py-2.5 border-b border-gray-700 flex items-center justify-between">
                ...
                <button onClick={() => setTryItResponse(null)} className="text-gray-500 hover:text-gray-300"><X className="w-4 h-4" /></button>
              </div>
