...
const getFileIcon = (name: string): { icon: React.ReactNode; color: string } => {
  ...
  const config: Record<string, { icon: React.ReactNode; color: string }> = {
    ...
    md: { icon: <FileText size={18} />, color: 'text-nexus-muted' },
    txt: { icon: <FileText size={18} />, color: 'text-nexus-muted' },
    log: { icon: <FileText size={18} />, color: 'text-nexus-muted' },
  };
  return config[ext || ''] || { icon: <File size={18} />, color: 'text-nexus-muted' };
};
    ...
    <div className="space-y-6 p-6">
      <FadeIn>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-nexus-muted mt-1">{totalSize.folders} folders · {totalSize.files} files</p>
          </div>
        ...
        <div className="bg-nexus-card rounded-2xl p-5 border border-nexus-border">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-nexus-text text-sm">Storage</span>
            </div>
            <span className="text-sm text-nexus-muted">45.2 GB / 100 GB used</span>
          </div>
          <div className="h-2 bg-nexus-surface rounded-full overflow-hidden mb-2">
      ...
      <FadeIn delay={0.1}>
        <div className="flex flex-col sm:flex-row gap-3">
          {/* Breadcrumb */}
          <div className="flex items-center gap-1 bg-nexus-card rounded-xl px-3 py-2 border border-nexus-border flex-1 overflow-x-auto">
            ...
            {currentPath.map((segment, i) => (
              <React.Fragment key={i}>
                <ChevronRight size={14} className="text-nexus-muted flex-shrink-0" />
                <button
                  onClick={() => navigateTo(i)}
                  className="text-sm text-nexus-text hover:text-blue-500 whitespace-nowrap"
                >
                  {segment}
                </button>
              </React.Fragment>
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setView('grid')} className={`p-2 rounded-lg ${view === 'grid' ? 'bg-nexus-surface' : ''}`}>
              <Grid3X3 size={14} className={view === 'grid' ? 'text-nexus-text' : 'text-nexus-muted'} />
            </button>
            <button onClick={() => setView('list')} className={`p-2 rounded-lg ${view === 'list' ? 'bg-nexus-surface' : ''}`}>
              <List size={14} className={view === 'list' ? 'text-nexus-text' : 'text-nexus-muted'} />
            </button>
          </div>
        </div>
      </FadeIn>
      ...
      {currentPath.length > 0 && (
        <button onClick={navigateUp} className="flex items-center gap-2 text-sm text-nexus-muted hover:text-nexus-text dark:hover:text-nexus-muted">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-nexus-border bg-nexus-surface/50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-nexus-muted uppercase cursor-pointer hover:text-nexus-text"
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold text-nexus-muted uppercase">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-nexus-border">
              {items.map((item, i) => (
                <motion.tr
                  ...
                  className="hover:bg-nexus-surface/60/30 cursor-pointer group"
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-nexus-text group-hover:text-blue-500">{item.name}</span>
                      {item.starred && <Star size={12} className="text-yellow-400 fill-yellow-400" />}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-nexus-muted">{item.size || `${item.children?.length || 0} items`}</td>
                  <td className="px-4 py-3 text-sm text-nexus-muted">{item.modified}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={e => e.stopPropagation()} className="p-1 rounded text-nexus-muted hover:text-nexus-muted"><Download size={14} /></button>
                      <button onClick={e => e.stopPropagation()} className="p-1 rounded text-nexus-muted hover:text-nexus-muted"><Edit3 size={14} /></button>
                      <button onClick={e => e.stopPropagation()} className="p-1 rounded text-nexus-muted hover:text-red-500"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {items.map((item, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              onClick={() => item.type === 'folder' ? navigateToFolder(item.name) : setSelectedFile(item)}
              className="bg-nexus-card rounded-xl p-4 border border-nexus-border cursor-pointer group text-center"
            >
              <div className={`text-3xl mb-2 mx-auto ${item.type === 'folder' ? 'text-blue-500' : item.color}`}>
                {item.icon}
              </div>
              <h4 className="text-sm font-medium text-nexus-text truncate group-hover:text-blue-500">{item.name}</h4>
              <p className="text-xs text-nexus-muted mt-1">{item.size || `${item.children?.length || 0} items`}</p>
            </motion.div>
          ))}
        </div>
      )}
      ...
      {items.length === 0 && (
        <div className="text-center py-16">
          <FolderOpen size={48} className="text-nexus-muted mx-auto mb-4" />
          <h3 className="text-lg font-medium text-nexus-text">Empty folder</h3>
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-nexus-text">File Details</h3>
                <button onClick={() => setSelectedFile(null)} className="p-1.5 rounded-lg hover:bg-nexus-surface">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h4 className="font-medium text-nexus-text">{selectedFile.name}</h4>
                  <p className="text-xs text-nexus-muted">{selectedFile.size}</p>
                </div>
              </div>
              <div className="space-y-2 mb-6">
                <div className="flex justify-between p-2 rounded bg-nexus-surface/50">
                  <span className="text-sm text-nexus-muted">Modified</span>
                  <span className="text-sm text-nexus-text">{selectedFile.modified}</span>
                </div>
                <div className="flex justify-between p-2 rounded bg-nexus-surface/50">
                  <span className="text-sm text-nexus-muted">Size</span>
                  <span className="text-sm text-nexus-text">{selectedFile.size}</span>
                </div>
              </div>
