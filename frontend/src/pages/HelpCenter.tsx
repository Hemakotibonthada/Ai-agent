    ...
    <div className="space-y-6 p-6">
      <FadeIn>
        <div className="text-center max-w-2xl mx-auto">
          <h1 className="text-3xl font-bold text-nexus-text flex items-center gap-3 justify-center">
            <BookOpen className="text-blue-500" size={32} />
            Help Center
          </h1>
          <p className="text-nexus-muted mt-2">Find answers, guides, and tutorials for NexusAI</p>

          {/* Search */}
          <div className="relative mt-6">
            <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-nexus-muted" />
            <input type="text" placeholder="Search for help articles, FAQs, tutorials..."
              value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-2xl bg-nexus-card border border-nexus-border text-nexus-text text-sm focus:outline-none focus:ring-2 focus:ring-nexus-primary" />
          </div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.05}>
        <div className="flex items-center justify-center gap-2 flex-wrap">
          <span className="text-xs text-nexus-muted">Popular:</span>
          {['setup guide', 'api keys', 'automations', 'voice commands', 'backup'].map(term => (
            <button key={term} onClick={() => setSearchQuery(term)}
              className="px-2.5 py-1 rounded-lg bg-nexus-surface text-xs text-nexus-muted hover:bg-nexus-surface transition-colors">
              {term}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {categories.map(cat => (
            <motion.button key={cat.id} whileHover={{ y: -4, scale: 1.02 }} whileTap={{ scale: 0.98 }}
              ...
              <div className="text-sm font-medium text-nexus-text">{cat.label}</div>
              <div className="text-xs text-nexus-muted mt-0.5">{cat.count} articles</div>
            </motion.button>
          ))}
        </div>
      </FadeIn>
            ...
            {selectedArticle ? (
              <FadeIn>
                <div className="bg-nexus-card rounded-2xl p-6 border border-nexus-border">
                  ...
                  <h2 className="text-2xl font-bold text-nexus-text mb-2">{selectedArticle.title}</h2>
                  <div className="flex items-center gap-4 text-xs text-nexus-muted mb-6">
                  ...
                  <div className="prose dark:prose-invert max-w-none">
                    <p className="text-nexus-text">{selectedArticle.summary}</p>
                    <p className="text-nexus-muted mt-4">{selectedArticle.content}</p>
                  ...
                  <div className="flex items-center gap-3 mt-8 pt-6 border-t border-nexus-border">
                    <span className="text-sm text-nexus-muted">Was this helpful?</span>
              ...
              <div className="space-y-3">
                {filteredArticles.map(article => (
                  <motion.div key={article.id} whileHover={{ x: 4 }}
                    ...
                    className="bg-nexus-card rounded-xl p-4 border border-nexus-border cursor-pointer hover:border-blue-300 dark:hover:border-blue-600 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <h3 className="text-sm font-semibold text-nexus-text">{article.title}</h3>
                        <p className="text-xs text-nexus-muted mt-1 line-clamp-1">{article.summary}</p>
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-[10px] text-nexus-muted flex items-center gap-1"><Clock size={10} /> {article.readTime}</span>
                          <span className="text-[10px] text-nexus-muted">{article.views.toLocaleString()} views</span>
                          {article.tags.map(t => (
                            <span key={t} className="px-1.5 py-0.5 rounded bg-nexus-surface text-[10px] text-nexus-muted">{t}</span>
                          ))}
                        </div>
                      </div>
                      <ChevronRight size={16} className="text-nexus-muted ml-3" />
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        {activeTab === 'faq' && (
          <motion.div key="faq" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            {filteredFaqs.map(faq => (
              <div key={faq.id}
                className="bg-nexus-card rounded-xl border border-nexus-border overflow-hidden">
                <button onClick={() => setExpandedFaq(expandedFaq === faq.id ? null : faq.id)}
                  className="w-full p-4 text-left flex items-center gap-3">
                  <ChevronDown size={14} className={`text-nexus-muted transition-transform flex-shrink-0 ${expandedFaq === faq.id ? 'rotate-180' : ''}`} />
                  <span className="text-sm font-medium text-nexus-text">{faq.question}</span>
                </button>
                <AnimatePresence>
                  {expandedFaq === faq.id && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }}
                      ...
                      <div className="px-4 pb-4 pl-11">
                        <p className="text-sm text-nexus-muted">{faq.answer}</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <motion.div key="tutorials" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            {tutorials.map(tutorial => (
              <motion.div key={tutorial.id} whileHover={{ y: -4 }}
                className="bg-nexus-card rounded-xl border border-nexus-border overflow-hidden cursor-pointer group">
                ...
                <div className="p-4">
                  <h3 className="text-sm font-semibold text-nexus-text">{tutorial.title}</h3>
                  <div className="flex items-center gap-3 mt-2">
                    <span className="text-xs text-nexus-muted flex items-center gap-1"><Clock size={10} /> {tutorial.duration}</span>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl p-6 text-white text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="px-4 py-2 bg-nexus-card text-blue-600 rounded-xl text-sm font-medium flex items-center gap-2">
              <MessageSquare size={14} /> Chat with AI
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="px-4 py-2 bg-nexus-card/20 text-white rounded-xl text-sm font-medium flex items-center gap-2">
