      ...
      <FadeIn>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-nexus-muted mt-1">{tables.length} tables · {totalRows.toLocaleString()} rows · {totalSize.toFixed(1)} MB</p>
          </div>
        ...
        {(['tables', 'query', 'stats'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveView(tab)}
            className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${
              activeView === tab ? 'bg-nexus-card text-nexus-text shadow-sm' : 'text-nexus-muted'
            }`}>
                  ...
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border transition-colors ${
                      selectedTable?.name === table.name
                        ...
                        : 'bg-nexus-card border-nexus-border hover:border-blue-200 dark:hover:border-blue-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Table2 size={14} className="text-blue-500 flex-shrink-0" />
                      <span className="text-sm font-medium text-nexus-text font-mono">{table.name}</span>
                      <ChevronRight size={12} className={`ml-auto text-nexus-muted transition-transform ${expandedTable === table.name ? 'rotate-90' : ''}`} />
                    </div>
                    <div className="flex gap-3 mt-1 text-[10px] text-nexus-muted">
                        ...
                        <div className="ml-4 mt-1 space-y-0.5">
                          {table.columns.map(col => (
                            <div key={col.name} className="flex items-center gap-2 px-2 py-1 text-xs">
                              ...
                              <span className="font-mono text-nexus-text">{col.name}</span>
                              <span className={`font-mono text-[10px] ${typeColors[col.type.split('(')[0]] || 'text-nexus-muted'}`}>
                                {col.type}
                              </span>
                              {col.nullable && <span className="text-nexus-muted text-[9px]">NULL</span>}
                            </div>
                          ))}
                        </div>
                      </motion.div>
            ...
            {selectedTable ? (
              <div className="bg-nexus-card rounded-2xl border border-nexus-border overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-nexus-border">
                  <div>
                    <h3 className="font-semibold text-nexus-text font-mono">{selectedTable.name}</h3>
                    <p className="text-xs text-nexus-muted">{selectedTable.rows.toLocaleString()} rows · {selectedTable.columns.length} columns · {selectedTable.size}</p>
                  </div>
                  <div className="flex gap-1">
                    <button className="p-2 rounded-lg hover:bg-nexus-surface text-nexus-muted"><Eye size={14} /></button>
                    <button className="p-2 rounded-lg hover:bg-nexus-surface text-nexus-muted"><Download size={14} /></button>
                    <button className="p-2 rounded-lg hover:bg-nexus-surface text-red-500"><Trash2 size={14} /></button>
                  </div>
                </div>
                <table className="w-full">
                  <thead>
                    <tr className="bg-nexus-surface/50">
                      <th className="px-4 py-2 text-left text-xs font-semibold text-nexus-muted">Column</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold text-nexus-muted">Type</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold text-nexus-muted">Nullable</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold text-nexus-muted">Key</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-nexus-border">
                    {selectedTable.columns.map(col => (
                      <tr key={col.name} className="hover:bg-nexus-surface/60/30">
                        <td className="px-4 py-2 text-sm font-mono text-nexus-text">{col.name}</td>
                        <td className={`px-4 py-2 text-sm font-mono ${typeColors[col.type.split('(')[0]] || 'text-nexus-muted'}`}>{col.type}</td>
                        <td className="px-4 py-2 text-sm text-nexus-muted">{col.nullable ? 'YES' : 'NO'}</td>
              ...
              <div className="bg-nexus-card rounded-2xl border border-nexus-border p-12 text-center">
                <Database size={48} className="text-nexus-muted mx-auto mb-4" />
                <h3 className="text-lg font-medium text-nexus-text">Select a Table</h3>
                <p className="text-sm text-nexus-muted mt-1">Choose a table from the sidebar to view its schema</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeView === 'query' && (
        <FadeIn delay={0.1}>
          <div className="space-y-4">
            <div className="bg-nexus-card rounded-2xl border border-nexus-border overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2 border-b border-nexus-border bg-nexus-surface/50">
                <span className="text-xs font-semibold text-nexus-muted">SQL Editor</span>
                <div className="flex gap-2">
                  <button className="text-xs text-nexus-muted hover:text-nexus-text">Format</button>
                  <button className="text-xs text-nexus-muted hover:text-nexus-text">Clear</button>
                </div>
              </div>
              <textarea value={query} onChange={e => setQuery(e.target.value)} rows={6}
                className="w-full px-4 py-3 bg-nexus-bg text-green-400 text-sm font-mono outline-none resize-none" />
              <div className="flex items-center justify-between px-4 py-2 border-t border-nexus-border">
                <span className="text-xs text-nexus-muted">Ctrl+Enter to execute</span>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={runQuery}
            ...
            {queryResult && (
              <div className="bg-nexus-card rounded-2xl border border-nexus-border overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2 border-b border-nexus-border">
                  <span className="text-sm font-semibold text-nexus-text">Results</span>
                  <div className="flex items-center gap-3 text-xs text-nexus-muted">
                    ...
                    <button className="flex items-center gap-1 hover:text-nexus-text"><Download size={10} /> CSV</button>
                    <button className="flex items-center gap-1 hover:text-nexus-text"><FileJson size={10} /> JSON</button>
                  </div>
                </div>
                    ...
                    <thead>
                      <tr className="bg-nexus-surface/50">
                        {queryResult.columns.map(col => (
                          <th key={col} className="px-4 py-2 text-left text-xs font-semibold text-nexus-muted font-mono whitespace-nowrap">{col}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-nexus-border">
                      {queryResult.rows.map((row, i) => (
                        <tr key={i} className="hover:bg-nexus-surface/60/30">
                          {queryResult.columns.map(col => (
                            <td key={col} className="px-4 py-2 text-sm font-mono text-nexus-text whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <FadeIn delay={0.1}>
            <div className="bg-nexus-card rounded-2xl p-5 border border-nexus-border">
              <h3 className="font-semibold text-nexus-text mb-4">Table Sizes (MB)</h3>
          ...
          <FadeIn delay={0.15}>
            <div className="bg-nexus-card rounded-2xl p-5 border border-nexus-border">
              <h3 className="font-semibold text-nexus-text mb-4">Top Tables by Rows</h3>
              <ResponsiveContainer width="100%" height={250}>
